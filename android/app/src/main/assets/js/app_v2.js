...
const OrisApp = {
            ...
            try {
                ...
                if (grid) {
                    grid.innerHTML = ''; // Clear
                        ...
                        { id: "init", icon: "assets/images/Iniciado.png", color: "#A87C7C" },
                        { id: "early", icon: "assets/images/Madrugador.png", color: "#7CA89B" },
                        { id: "night", icon: "assets/images/Nocturno.png", color: "#5F6B8A" },
                        { id: "fifty", icon: "assets/images/Perseverante.png", color: "#A8987C" },
                        { id: "moon", icon: "assets/images/Dador.png", color: "#939CA9" },
                        { id: "alchemist", icon: "assets/images/Alquimista.png", color: "#4E876A" },
                        { id: "compassive", icon: "assets/images/Compasivo.png", color: "#5A8BB5" },
                        { id: "grateful", icon: "assets/images/Agradecido.png", color: "#D4B85A" },
                        { id: "sincere", icon: "assets/images/Sincero.png", color: "#D4845A" },
                        { id: "humble", icon: "assets/images/Humilde.png", color: "#7B5EA7" }
                    ];
                    ...
                    ACHIEVEMENTS_DATA.forEach(ach => {
                        ...
                        if (unlockedArray.indexOf(ach.id) !== -1) isUnlocked = true;
                        ...
                        const bgColor = isUnlocked ? ach.color + '26' : '#e0e0e0';

                        card.innerHTML = 
                            '<div class="achievement-icon" style="background-color: ' + bgColor + '; border-radius: 50%; width: 48px; height: 48px; display: flex; align-items: center; justify-content: center; margin: 0 auto 4px;">' +
                                '<div style="-webkit-mask-image: url(\'' + ach.icon + '\'); mask-image: url(\'' + ach.icon + '\'); -webkit-mask-size: contain; mask-size: contain; -webkit-mask-repeat: no-repeat; mask-repeat: no-repeat; -webkit-mask-position: center; mask-position: center; background-color: ' + iconColor + '; width: 28px; height: 28px;"></div>' +
                            '</div>' +
                        ...
                        card.addEventListener('click', () => {
                            ...
                            iconContainer.innerHTML = '<div style="-webkit-mask-image: url(\'' + ach.icon + '\'); mask-image: url(\'' + ach.icon + '\'); -webkit-mask-size: contain; mask-size: contain; -webkit-mask-repeat: no-repeat; mask-repeat: no-repeat; -webkit-mask-position: center; mask-position: center; background-color: ' + iconColor + '; width: 40px; height: 40px;"></div>';
                            ...
                            const lockedKey = isUnlocked ? 'achievements.status_unlocked' : 'achievements.status_locked';
                            let lockedStr = (typeof Translations !== 'undefined' && Translations[this.currentLang] && Translations[this.currentLang][lockedKey]) 
                                            ? Translations[this.currentLang][lockedKey] 
                                            : (isUnlocked ? 'DESBLOQUEADO' : 'BLOQUEADO');
                            statusEl.textContent = lockedStr.toUpperCase();
  ...
  onTimerComplete() {
    ...
    setTimeout(() => {
      ...
      // Setup orbiting achievements text
      const orbitContainer = document.getElementById('sigil-orbit-container-svg');
      if (orbitContainer) {
          orbitContainer.querySelectorAll('text').forEach(t => t.remove());
          const ACHIEVEMENTS_DATA = [
          ...
          ACHIEVEMENTS_DATA.forEach((ach, index) => {
              ...
              // Shift by 5% so the text doesn't overlap the start/end point of the SVG path when anchored at middle
              ...
              textPathEl.setAttributeNS("http://www.w3.org/1999/xlink", "href", "#orbit-path");
              textPathEl.setAttribute("startOffset", offset);
              textPathEl.setAttribute("text-anchor", "middle");
              textPathEl.textContent = titleText;
              
              textEl.appendChild(textPathEl);
              orbitContainer.appendChild(textEl);
          });
          
          orbitContainer.classList.remove('orbit-animate');
      }
      ...
      const orbitContainerSvg = document.getElementById('sigil-orbit-container-svg');
      if (orbitContainerSvg) {
          setTimeout(() => {
              orbitContainerSvg.classList.add('orbit-animate');
          }, 50); // Small delay to ensure the DOM has updated from display: none
      }
    }, 800);
  },
...
,
    ...
    startUniverseSearch() {
        ...
        if (Math.random() <= 0.40) { // 40% probability
...
window.achievementSystem = {
    ...
    check: function(historyItem, history) {
        ...
        // Cheat codes
        if (historyItem.text) {
            const allAchievements = ["init", "early", "night", "fifty", "moon", "alchemist", "compassive", "grateful", "sincere", "humble"];
            let t = historyItem.text.toUpperCase();
            if (t.includes('SEMIGODMODE')) {
                localStorage.setItem('oris_achievements', JSON.stringify([]));
                const shuffled = allAchievements.sort(() => 0.5 - Math.random());
                const selected = shuffled.slice(0, 5);
                selected.forEach(id => this.unlock(id));
                return;
            } else if (t.includes('GODMODE')) {
                localStorage.setItem('oris_achievements', JSON.stringify([]));
                allAchievements.forEach(id => this.unlock(id));
                return;
            } else if (t.includes('POORMODE')) {
                localStorage.setItem('oris_achievements', JSON.stringify([]));
                return;
            }
        }
        ...
        // Check frequency-based achievements (requires 10 channelings of a specific frequency)
        let humilisCount = 0;
        let revelatioCount = 0;
        let absolutioCount = 0;
        let gratiaCount = 0;
        
        for (let i = 0; i < history.length; i++) {
            let item = history[i];
            if (item.type === 'humilis') humilisCount++;
            if (item.type === 'revelatio') revelatioCount++;
            if (item.type === 'absolutio') absolutioCount++;
            if (item.type === 'gratia') gratiaCount++;
        }
        
        if (absolutioCount >= 10) this.unlock('compassive');
        if (gratiaCount >= 10) this.unlock('grateful');
        if (revelatioCount >= 10) this.unlock('sincere');
        if (humilisCount >= 10) this.unlock('humble');
    },
