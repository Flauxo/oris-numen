...
try {
    const file_path = 'C:\\\\Users\\\\Kivan\\\\.gemini\\\\antigravity\\\\scratch\\\\oris-numen\\\\js\\\\translations.js';
    let raw = fs.readFileSync(file_path, 'utf-8');
    
    // Evaluate the object
    let objStr = raw.replace('const Translations = ', '').trim();
    if (objStr.endsWith(';')) {
        objStr = objStr.slice(0, -1);
    }
    
    let Translations = eval('(' + objStr + ')');
    
    // Inject proverbs
    for (let lang of ['en', 'es', 'it', 'la', 'zh']) {
        let provs = data[lang];
        for (let freq in provs) {
            let stmts = provs[freq];
            for (let i = 0; i < stmts.length; i++) {
                Translations[lang][`proverb.${freq}.${i+5}`] = stmts[i];
            }
        }
    }
    ...
    console.log('Successfully injected proverbs via Node!');
} catch (e) {
    console.error(e);
}
