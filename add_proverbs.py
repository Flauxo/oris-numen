...
# New proverbs to add (indices 5 to 9)
proverbs_en = {
    "humilis": [
...
# For each language, find the section and append
    ...
    for freq, statements in provs.items():
        ...
        if match:
            ...
            for i, stmt in enumerate(statements):
                idx = i + 5
                # if it is the very last proverb in the json, it shouldn't have a trailing comma, 
                # but these are inserted inside the object so trailing comma is fine unless it's the absolute end
                # but it's safe to always add comma since it's just JS object syntax (which allows trailing commas, but JSON doesn't).
                # translations.js is just a JS object, trailing commas are completely valid.
                new_lines += f'        "proverb.{freq}.{idx}": "{stmt}",\n'
            
            content = content[:match.end()] + new_lines + content[match.end():]
        else:
            print(f"Failed to find {target} in language {lang}")

# Write back
with open(file_path, "w", encoding="utf-8") as f:
    f.write(content)

print("Done inserting proverbs!")
